        ...
        <div class="w-full md:w-2/3 px-2">
              ...
              <div>
                <div class="md:inline-flex py-3 border-t border-solid border-[#e1e1e1]">
                  <div class="md:w-3/5 md:pr-4 mb-3">
                    ...
                    <br>
                    <div class="leading-5">
                      <a href="{{ clientRoute('page_news_detail', ['permalink' => $newHot1->permalink]) }}" class="text-[#2C5D6C] hover:text-[#2C5D6C] text-[20px] font-bold
                  ...
                  <div class="md:w-2/5">
                    <div>
                      @foreach($newHot2 as $item)
                        <div class="flex pb-3">
                          <a class="w-1/3 block rounded" href="{{ clientRoute('page_news_detail', ['permalink' => $item->permalink]) }}">
              ...
              <div class="inline-flex py-3 border-t border-solid border-[#e1e1e1] w-full">
                <div class="w-1/3">
                  <a href="{{ clientRoute('page_news_detail', ['permalink' => arrayGet($item, 'slug')]) }}">
                    <img src="{{ !empty($item['avatar']['src']) ? $item['avatar']['src'] : asset('client/image/default.jpg') }}"
                         alt="{{ !empty($item['avatar']['alt']) ? $item['avatar']['alt'] : "Alt image" }}"
                ...
                <div class="w-2/3 px-4 py-0">
                  ...
                  <small class="block my-2 text-[#9F9E9A]">
                    Ngày đăng: {{ date('d-m-Y H:i:s', strtotime(arrayGet($item, 'date'))) }} <br>
                    Danh mục:
                    @foreach(arrayGet($item, 'category', []) as $k => $c)
                      <a class="hover:text-[#2C5D6C]" href="{{ clientRoute('page_danh_muc_tin_tuc_detail', ['permalink' => arrayGet($c, 'slug')]) }}">{{ arrayGet($c, 'name') }}</a>
                      @if ($k != count(arrayGet($item, 'category', [])) - 1) <span>, </span> @endif
                    @endforeach
                  </small>
                  <span class="text-[14px] ellipsis-text-2 text-justify">
          ...
          <!-- Pagination -->
            <div class="mt-3 text-center">
              {{ $dataList->withPath(url()->current())->links('vendor.pagination.bootstrap-4') }}
            </div>
          </div>
        </div>
      </div>
