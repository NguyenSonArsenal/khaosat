@extends('wowtop.layout.main')
@section('title', 'Danh mục tin tức')

@push('styles')
  <link rel="stylesheet" href="{{ asset('wowtop/assets/css/san-pham.css') }}">
  <link rel="stylesheet" href="{{ asset('wowtop/assets/css/tin-tuc.css') }}">
  <link rel="stylesheet" href="{{ asset('wowtop/assets/css/_pagination.css') }}">
@endpush

@section('content')
  <section class="section-main section-benh-thuong-gap pb-8">
    <div class="wrapper">
      <div class="">
        <div class="w-full lg:w-3/4  px-2">
          <div class="text-[30px] text-[#2C5D6C] font-semibold my-3">{{ $categoryNew->name }}</div>
          @if (count($dataList) == 0)
            <div>Danh sách trống</div>
          @endif
          <div class="">
            @foreach($dataList as $item)
              <div class="inline-flex py-3 border-t border-solid border-[#e1e1e1] w-full align-items-center">
                <div class="w-1/3">
                  <a href="{{ clientRoute('page_news_detail', ['permalink' => arrayGet($item, 'slug')]) }}">
                    <img src="{{ !empty($item['avatar']['src']) ? $item['avatar']['src'] : asset('client/image/default.jpg') }}"
                         alt="{{ !empty($item['avatar']['alt']) ? $item['avatar']['alt'] : "Alt image" }}"
                         class="rounded-xl w-[260px] h-[135px] object-cover">
                  </a>
                </div>
                <div class="w-2/3 px-4 py-0 content_info">
                  <div class="leading-5">
                    <a href="{{ clientRoute('page_news_detail', ['permalink' => arrayGet($item, 'slug')]) }}"
                       class="text-[#2C5D6C] hover:text-[#2C5D6C] text-[20px] font-bold
                       leading-tight md:leading-normal ellipsis-text-3">
                      {{ arrayGet($item, 'title') }}
                    </a>
                  </div>
                  <small class="block my-2 text-[#9F9E9A]">
                    {{ date('d-m-Y H:i:s', strtotime(arrayGet($item, 'date'))) }}
                  </small>
                  <div class="ellipsis-text-2 text-[14px] text-justify">
                    {!! !empty($item['excerpt']['rendered']) ? $item['excerpt']['rendered'] : ''  !!}
                  </div>
                </div>
              </div>
            @endforeach

            <!-- Pagination -->
              <div class="mt-3 text-center">
                {{ $dataList->withPath(url()->current())->links('vendor.pagination.bootstrap-4') }}
              </div>
          </div>
        </div>
      </div>
    </div>
  </section>
@endsection

